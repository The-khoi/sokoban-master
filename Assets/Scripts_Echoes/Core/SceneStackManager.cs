...
namespace Echoes.Core
{
    ...
    public class SceneStackManager : MonoBehaviour
    {
        ...
        private IEnumerator UnloadInnerSceneCoroutine()
        {
            ...
            // 3. [Echoes Mod]: 内层场景即将卸载，无需冻结，直接清理 SceneStateController 快照
            Scene currentScene = SceneManager.GetSceneByName(sceneName);
            if (currentScene.isLoaded)
            {
                SceneStateController stateController = FindSceneStateController(currentScene.GetRootGameObjects());
        ...
        /// <summary>
        /// [Echoes Mod]: 暂停场景的输入响应与物理模拟。
        /// 分两层处理：
        ///   1. 禁用输入控制器（PlayerMovementController / GameController / EchoesInputController）
        ///   2. 通过 SceneStateController 冻结物理（Rigidbody2D.simulated + Collider2D.enabled）
        /// </summary>
        private void SuspendScene(Scene scene)
        {
            ...
            // --- 第二层：[Echoes Mod] 通过 SceneStateController 冻结物理 ---
            SceneStateController stateController = FindSceneStateController(rootObjects);
            if (stateController != null)
            {
                // 收集场景内所有根对象作为冻结目标列表
                var levelObjects = new List<GameObject>(rootObjects);
                stateController.FreezeScene(levelObjects);
            }
            else
            {
                Debug.LogWarning($"[SceneStackManager] 场景 '{scene.name}' 未找到 SceneStateController，" +
                                 "物理冻结跳过。请在场景管理节点上挂载该组件。");
            }
        }
        
        /// <summary>
        /// [Echoes Mod]: 恢复场景的输入响应与物理模拟。
        /// 分两层处理：
        ///   1. 重新启用输入控制器
        ///   2. 通过 SceneStateController 解冻物理，精确还原冻结前状态
        /// </summary>
        private void ActivateScene(Scene scene)
        {
            ...
            // --- 第二层：[Echoes Mod] 通过 SceneStateController 解冻物理 ---
            SceneStateController stateController = FindSceneStateController(rootObjects);
            if (stateController != null)
            {
                var levelObjects = new List<GameObject>(rootObjects);
                stateController.UnfreezeScene(levelObjects);
            }
            // 未找到 SceneStateController 时静默处理（场景可能本就没有物理对象）
        }
        
        /// <summary>
        /// [Echoes Mod]: 在根对象列表中查找 SceneStateController 组件
        /// </summary>
        private SceneStateController FindSceneStateController(GameObject[] rootObjects)
        {
            foreach (var obj in rootObjects)
            {
                var controller = obj.GetComponentInChildren<SceneStateController>(true);
