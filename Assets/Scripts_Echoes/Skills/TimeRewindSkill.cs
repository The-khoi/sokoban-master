...
namespace Echoes.Core
{
    ...
    public class TimeRewindSkill : Skill
    {
        ...
        /// - 时间回溯(Time Rewind)：多步回退，通过 CommandHistoryManager.UndoSteps，消耗能量
        /// 执行时间回溯时需要清空两套撤销历史，避免状态不一致
        /// </summary>
        protected override void ExecuteSkill()
        {
            ...
            // [Echoes Mod]: 优先通过 CommandHistoryManager 执行精确的多步撤销（最多3步）
            // 这比直接恢复快照更精确，因为每一步都经过 MoveCommand.Undo() 的位置还原
            if (CommandHistoryManager.Instance != null && CommandHistoryManager.Instance.CanUndo)
            {
                int stepsUndone = CommandHistoryManager.Instance.UndoSteps(3);
                Debug.Log($"[TimeRewindSkill] 通过 CommandHistoryManager 回溯了 {stepsUndone} 步。");
                
                // 同步清空 GameStateRecorder，保持两套系统一致
                GameStateRecorder.Instance.Clear();
            }
            else
            {
                // 降级方案：CommandHistoryManager 不可用时，回退到快照恢复
                Debug.LogWarning("[TimeRewindSkill] CommandHistoryManager 不可用，降级为快照恢复。");
            ...
            // [Echoes Mod]: 同步清空模板原有的 CommandHistoryHandler，避免状态不一致
            ...
            Debug.Log("[TimeRewindSkill] Time rewound successfully! All command histories cleared.");
        }
